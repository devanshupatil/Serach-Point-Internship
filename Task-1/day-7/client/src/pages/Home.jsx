...
const Home = () => {
  ...
  const loadData = async () => {
    try {
      if (activeTab === 'home' || activeTab === 'timeline') {
        ...
        const res = await getStarredItems();
        if (res.success) setStarredItems(res.data);
      } else if (activeTab === 'archived') {
        const res = await getArchivedItems();
        if (res.success) setArchivedItems(res.data);
      } else if (activeTab === 'reminders') {
        const res = await getReminders();
        if (res.success) setReminderItems(res.data);
      }
    } catch (error) {
  ...
  const handleArchiveItem = async (item) => {
    try {
      await updateItem(item._id, { isArchived: !item.isArchived });
      loadData();
    } catch (error) {
  ...
  const renderItemRow = (item) => (
    <div key={item._id} className="p-4 flex items-start gap-4 hover:bg-slate-50 transition group">
      ...
      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
        <button onClick={() => handleStarItem(item)} title={item.isStarred ? "Unstar" : "Star"} className="text-yellow-500 hover:text-yellow-600">
          {item.isStarred ? '⭐' : '☆'}
        </button>
        <button onClick={() => handleArchiveItem(item)} title={item.isArchived ? "Restore from Archive" : "Archive"} className="text-blue-500 hover:text-blue-600">
          {item.isArchived ? '📤' : '📦'}
        </button>
        <button onClick={() => handleEditItem(item)} title="Edit" className="text-slate-500 hover:text-slate-600">✏️</button>
        <button onClick={() => handleMoveToTrash(item)} title="Move to Trash" className="text-red-500 hover:text-red-600">🗑️</button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-wrap gap-2 mb-8">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium text-sm transition flex items-center gap-2 ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'}`}
            >
              <span>{tab.icon}</span>
              {tab.label}
            </button>
          ...
          <div className="space-y-8">
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowFolderInput(true)} className="text-sm text-blue-600 hover:text-blue-700 font-medium">+ New Folder</button>
              </div>
              {showFolderInput && (
        ...
        {activeTab === 'starred' && (
          <section>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
              {starredItems.map(renderItemRow)}
        ...
        {activeTab === 'archived' && (
          <section>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
              {archivedItems.map(renderItemRow)}
              {archivedItems.length === 0 && <p className="p-8 text-center text-slate-500">No archived items</p>}
            </div>
          </section>
        )}
        ...
        {activeTab === 'reminders' && (
          <section>
            <h2 className="text-lg font-semibold text-slate-800 mb-4">🔔 Upcoming Reminders</h2>
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
              {reminderItems.map(renderItemRow)}
              {reminderItems.length === 0 && <p className="p-8 text-center text-slate-500">No reminders set</p>}
            </div>
          </section>
        )}
