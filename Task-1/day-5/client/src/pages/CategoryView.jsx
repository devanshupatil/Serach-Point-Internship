...
const CategoryView = () => {
  ...
  const getTypeLabel = () => {
    const labels = { image: 'Images', document: 'Documents', link: 'Links', video: 'Videos', note: 'Notes' };
  ...
  const renderImagesGrid = () => (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      {items.map((item) => (
        <div key={item._id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition group relative">
          <div className="aspect-square bg-slate-100 flex items-center justify-center">
            {item.metadata?.url ? <img src={item.metadata.url} alt={item.title} className="w-full h-full object-cover" /> : <span className="text-4xl">🖼️</span>}
          </div>
          <div className="p-3">
      ...
      {items.length === 0 && <div className="col-span-full text-center py-12 text-slate-500">No images yet</div>}
    </div>
  );
  ...
  const renderDocumentsList = () => (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 divide-y divide-slate-100">
      ...
      {items.length === 0 && <div className="p-12 text-center text-slate-500">No documents yet</div>}
    </div>
  );
  ...
  const renderLinksPreview = () => (
    <div className="grid gap-4">
      ...
      {items.length === 0 && <div className="text-center py-12 text-slate-500">No links yet</div>}
    </div>
  );
  ...
  const renderVideosEmbedded = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.map((item) => (
        <div key={item._id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition group">
          <div className="aspect-video bg-slate-900 flex items-center justify-center">
            {item.metadata?.url ? (item.metadata.url.includes('youtube') || item.metadata.url.includes('youtu.be') ? <iframe src={item.metadata.url.replace('watch?v=', 'embed/')} className="w-full h-full" allowFullScreen title={item.title} /> : <video src={item.metadata.url} controls className="w-full h-full object-contain" />) : <span className="text-4xl">🎬</span>}
          </div>
          <div className="p-4">
            <p className="font-medium text-slate-800 truncate">{item.title}</p>
      ...
      {items.length === 0 && <div className="col-span-full text-center py-12 text-slate-500">No videos yet</div>}
    </div>
  );
  ...
  const renderNotesList = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {items.length === 0 && <div className="col-span-full text-center py-12 text-slate-500">No notes yet</div>}
    </div>
  );
